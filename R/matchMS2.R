...
matchMS2_main <- function(object , ppm = 30 ,ms2checkresult_name = "CL" ,workers = 2){
  dealing_list <- MS2checkresult@ms2CheckResult[[ms2checkresult_name]]
  #将待处理的列表输出
  paste0("Searing and splicing from: ",ms2checkresult_name) %>%
    message()
  future::plan(multisession , workers = workers)
  #设置多线程使用核数
  # headGroupResult <- furrr::future_pmap(.l = list(dealing_list , ms2checkresult_name))
  headGroupResult <- furrr::future_map(.x = dealing_list , .f = findmyCL::check_MS1_MS2_HeadFAPA , ppm = ppm , ms2checkresult_name = ms2checkresult_name)
  # headGroupResult <- purrr::map(.x = dealing_list , .f = findmyCL::check_MS1_MS2_HeadFAPA , ppm = ppm)
  #迭代输出结果,检查了头基并且拼接了心磷脂
  cat('\n')
  #换行
  message("Done!")
  #提示信息
  headGroupResult <- findmyCL::deleteNULL(list = headGroupResult)
  #去除空值
  if (length(headGroupResult) == 0) {
    return(NULL)
  }
  #如果都没有头基，则直接将这个结果删除
  names(headGroupResult) <- 1:length(headGroupResult)
  #重命名结果
  return(headGroupResult)
}
#配对ms2checkresult中比对的每个结果,比如“CL”结果,然后对结果进行拼接成心磷脂。
...
#' @param pb 1
#' @export
check_MS1_MS2_HeadFAPA <- function(MS1, ppm = 30 , ms2checkresult_name){
  MS1_dealing <- MS1[-1]
  #去除一级的信息
  result <- purrr::map(.x = MS1_dealing,.f = findmyCL::check_HeadFAPA,ppm = ppm,MS1 = MS1$MS1 , ms2checkresult_name = ms2checkresult_name)
  ...
  if (length(result) > 0) {
    store <- list()
    #存储结果
    store <- findmyCL::fappend(store,MS1[[1]])
    #追加一级的信息
